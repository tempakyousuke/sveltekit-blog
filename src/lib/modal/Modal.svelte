...
>
	...
	>
		...
		<div class="modal-content text-left overflow-y-hidden">
			<!-- Title-->
			<div class="flex justify-between items-center p-4 bg-blue-500 text-white">
				<p class="text-2xl font-bold">{title}</p>
				<button class="modal-close cursor-pointer z-50" on:click={() => (open = false)}>
					<svg
			...
			<slot name="footer">
				<div class="flex justify-end p-6">
					...
					<div class="w-24">
						{#if !hideCancel}
							<Button bgColorClass="bg-blue-100" block on:click={() => (open = false)}>
								{cancelText}
							</Button>
						{/if}
					</div>
